...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Log statistics export (client-side export, so we need a separate log endpoint)
  app.post("/api/statistics/log-export", requireAdmin, async (req, res) => {
    try {
      const { stationId, stationName, period } = req.body;
      
      await logActivity({
        userId: req.user!.id,
        stationId: stationId || req.user!.stationId,
        action: ActivityActions.EXPORT.STATISTICS_EXPORTED,
        category: 'EXPORT',
        details: `Statistieken geëxporteerd voor ${stationName || 'onbekend station'} - periode: ${period || 'onbekend'}`,
        ipAddress: getClientInfo(req).ipAddress,
        userAgent: getClientInfo(req).userAgent
      });
      ...
      console.error("Error logging statistics export:", error);
      res.status(500).json({ message: "Failed to log statistics export" });
    }
  });
