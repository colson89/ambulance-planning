...
const outputFile = process.argv[3] || 'scripts/import-data.sql';
...
output.push('');
output.push('-- Disable foreign key checks temporarily');
...
for (let i = 0; i < lines.length; i++) {
  ...
  if (line.startsWith('COPY public.')) {
    ...
    if (line === '\\.') {
      ...
      if (values.length === tableData[currentTable].columns.length) {
...
output.push('-- Delete existing data in reverse order');
...
for (const table of tableOrder) {
  ...
  for (const row of data.rows) {
    ...
    output.push(`INSERT INTO ${table} (${data.columns.join(', ')}) VALUES (${values.join(', ')});`);
  }
  output.push('');
}
...
for (const table of tableOrder) {
  ...
  if (data && data.rows.length > 0 && data.columns[0] === 'id') {
    output.push(`SELECT setval('${table}_id_seq', (SELECT COALESCE(MAX(id), 1) FROM ${table}));`);
  }
}
output.push('');
